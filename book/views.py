...
class TotalReadingTime(APIView):
    def get(self, request, book_id):
        total_reading_time = ReadingSession.objects.filter(book_id=book_id).aggregate(
            total=Sum(ExpressionWrapper(F("end_time") - F("start_time"), output_field=fields.DurationField()))
        )
...
class UserStatistics(APIView):
    def get(self, request):
        if request.user.is_authenticated:
            ...
            total_duration = sum((session.duration for session in reading_sessions), timedelta())
